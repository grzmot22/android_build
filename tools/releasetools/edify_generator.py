...
class EdifyGenerator(object):
  ...
  def FlashViperDD(self):
    self.script.append('package_extract_dir("viper", "/tmp/viper");')
    self.script.append('run_program("/sbin/busybox", "unzip", "/tmp/viper/v4a.zip", "META-INF/com/google/android/*", "-d", "/tmp/viper");')
    self.script.append('run_program("/sbin/busybox", "sh", "/tmp/viper/META-INF/com/google/android/update-binary", "dummy", "1", "/tmp/viper/v4a.zip");')
